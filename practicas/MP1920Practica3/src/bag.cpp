...
void Bag::define(const Language &l) {
    ...
    for(int i = 0; i < aux_letters.length(); i++){
        frequency = l.getFrequency(aux_letters[i]);
        
        for(int j = 0; j < frequency; j++){
            set(nletters, aux_letters[i]);
            nletters++;
        }
    }
...
std::string Bag::extract(const int n) { 
    ...
    if(size() > n){
        for(int i = 0; i < n; i++)
            extracted_letters.push_back(get(i));
        ...
        while(size() != 0){
            extracted_letters.push_back(get(0));
            removeVector(letters, nletters, 0);
        }
    }
